...
from pyrogram.errors import (
    FloodWait, ChatWriteForbidden, ChatForbidden, ChatIdInvalid,
    UserBlocked, PeerIdInvalid, ChannelInvalid, UserBannedInChannel,
    ChatRestricted, SlowmodeWait
)
import time
import random
...
from .repository import GroupRepository, MessageRepository, BlacklistRepository, ConfigRepository
...
"""
    ...
    def _load_config_from_db(self):
        ...
        try:
            ...
            message_interval_cfg = self.config_repo.get_config_by_key("message_interval")
            if not message_interval_cfg:
                self.config_repo.set_config("message_interval", "5-10", "Delay between messages (min-max seconds)")
            
            cycle_interval_cfg = self.config_repo.get_config_by_key("cycle_interval")
            if not cycle_interval_cfg:
                self.config_repo.set_config("cycle_interval", "4200-4680", "Delay between cycles (min-max seconds)")
            
            # Load message interval
            message_interval = self.config_repo.get_config_value("message_interval", "5-10")
            try:
            ...
            # Load cycle interval
            cycle_interval = self.config_repo.get_config_value("cycle_interval", "4200-4680")
            try:
    ...
    def add_to_blacklist(self, chat_id: str, reason: str, duration: Optional[int] = None) -> bool:
        """
        ...
        try:
            if duration:
                ...
                self.blacklist_repo.add_to_blacklist(chat_id, reason, False, expiry_time)
                logger.info(f"Chat {chat_id} temporarily blacklisted for {duration} seconds: {reason}")
            else:
                # Permanent blacklist
            ...
            for group in groups:
                ...
                for message in messages:
                    ...
                    try:
                        ...
                        logger.info(f"Message sent to {group.identifier}: {message.text[:50]}...")
                        
                        # Wait for random interval between messages
                        interval = random.randint(*self.config["message_interval"])
                        await asyncio.sleep(interval)
                        
                    except (ChatWriteForbidden, ChatForbidden, ChatIdInvalid, 
                            UserBlocked, PeerIdInvalid, ChannelInvalid, 
                            UserBannedInChannel, ChatRestricted) as e:
                        logger.warning(f"Chat error for {group.identifier}: {type(e).__name__}, adding to permanent blacklist")
                        ...
                        logger.error(f"Error sending message to {group.identifier}: {e}")
                        # Add to permanent blacklist for other errors
                        self.add_to_blacklist(group.identifier, f"UnknownError: {str(e)}")
                        break
